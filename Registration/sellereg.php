...
<body>
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit']) && $_POST['otpVerified'] === "true") {
    ...
    $stmt = $conn->prepare("SELECT email, mobile_number, nid_number FROM sellersinfo WHERE email = ? OR mobile_number = ? OR nid_number = ?");
    ...
    if ($result->num_rows > 0) {
      ...
      if (
        ...
        $seller_picture_extension = pathinfo($_FILES['own_picture']['name'], PATHINFO_EXTENSION);
        $nid_picture_extension = pathinfo($_FILES['nid_picture']['name'], PATHINFO_EXTENSION);
        $seller_picture_newname = "sellerpic/" . $mobile_number . "." . $seller_picture_extension;
        $nid_picture_newname = "sellernid/" . $mobile_number . "." . $nid_picture_extension;

        // Ensure directories exist
        if (!file_exists("sellerpic/")) {
          mkdir("sellerpic/", 0777, true);
        }
        if (!file_exists("sellernid/")) {
          mkdir("sellernid/", 0777, true);
        }

        // Move files
        move_uploaded_file($_FILES["own_picture"]["tmp_name"], $seller_picture_newname);
  ...
  if ($registration_success) {
    ...
    echo '<div class="flex justify-center mt-4">';
    echo '<button onclick="window.location=\'./sellereg.php\';" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">Close</button>';
    echo '</div>';
    echo '</div>';
    echo '</dialog>';
