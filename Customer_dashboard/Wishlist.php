...
if ($userId) { // Check if the user is logged in
    ...
    $cartQuery = "SELECT wishlist_id FROM wishlist WHERE customer_id = ?";
    if ($stmt = $conn->prepare($cartQuery)) {
        ...
        if ($result->num_rows > 0) {
            ...
            $itemsQuery = "SELECT ci.wishlist_item_id, ci.product_id AS Product_id, pi.Product_name, pi.Product_Description, pi.New_price, pi.Product_image_path
               FROM wishlist_items ci
               JOIN product_info pi ON ci.product_id = pi.Product_id
               WHERE ci.wishlist_id = ?";

            if ($itemsStmt = $conn->prepare($itemsQuery)) {
                $itemsStmt->bind_param("i", $wishlist_id);
                $itemsStmt->execute();
                ...
                if ($itemsResult->num_rows > 0) {
                    ...
                    $errorMsg = "No products in your Wishlist.";
                }
                $itemsStmt->close();
            }
        } else {
            $errorMsg = "No Wishlist found for this user.";
        }
        $stmt->close();
    }
} else {
...
<span class="text-lg font-medium text-gray-900">{$product['Product_name']}
            ...
            <button class="text-red-500 hover:text-red-700 p-2" onclick="deleteCartItem({$product['wishlist_item_id']})">
...
HTML;
    ...
    <script>
        // Access DOM elements directly (without waiting for DOMContentLoaded)
        ...
        // Add event listeners to delete buttons for each item
        ...
        // Function to update the order summary (subtotal, shipping, total)
        ...
        // Add listener to the "viewDetails" button to toggle the modal
        ...
        function deleteCartItem(cartItemId) {
            fetch('../Wishlist/delete_wishlist_item.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                    },
                    body: 'wishlist_item_id=' + cartItemId
                })
