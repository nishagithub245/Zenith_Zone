...
$customerId = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : null; // Retrieve customer ID from session
$errorMsg = '';
$walletDetails = null;
...
if ($customerId) {
    ...
    if ($stmt = $conn->prepare($walletQuery)) {
        $stmt->bind_param("i", $customerId); // Bind the customer_id to the query
        $stmt->execute();
        ...
        if ($result->num_rows > 0) {
            ...
            if ($stmtCreate = $conn->prepare($createWalletQuery)) {
                $stmtCreate->bind_param("i", $customerId); // Bind the customer_id to the query
        ...
        <?php if (!empty($walletDetails)) { ?>
            <div class="bg-white rounded-lg shadow-lg p-6 max-w-sm mx-auto">
                ...
                <div class="mt-6 text-center" id="addFund" >
                    <a class="btn btn-sm btn-primary">Add Funds</a>
                </div>
            </div>
        <?php } else { ?>
