...
$sellerId = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : null; // Retrieve seller ID from session
$errorMsg = '';
$walletDetails = null;
...
if ($sellerId) {
    ...
    if ($stmt = $conn->prepare($walletQuery)) {
        $stmt->bind_param("i", $sellerId); // Bind the seller_id to the query
        $stmt->execute();
        ...
        if ($result->num_rows > 0) {
            ...
            $createWalletQuery = "
            INSERT INTO seller_wallet (seller_id, balance) 
            VALUES (?, 0.00)"; // Initialize balance as 0.00

            if ($stmtCreate = $conn->prepare($createWalletQuery)) {
                $stmtCreate->bind_param("i", $sellerId); // Bind the seller_id to the query
